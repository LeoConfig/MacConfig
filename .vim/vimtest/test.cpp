...
*        Create: 2018-12-02 12:27:50
# Last Modified: 2018-12-02 21:55:44
*   Description: ---
*
************************************************/
...
int main()
{
    ...
    auto it = find_if_not(begin(vectorint1),end(vectorint1),[](int i)
    {
        return i<8;
    });    //搜索算法
    for_each(begin(vectorint1),end(vectorint1),[&sum](int i)
    {
        sum+=i;
    });                 //操作算法，每个元素执行回调函数
    partition(begin(vectorint1),end(vectorint1),[](int i)
    {
        return i%2;
    });                //分区算法  一个目标到两个目标  通过返回值 true、false决定区间//lambda表达式，返回true的排在前面   返回false的排在后面
