...
*        Create: 2018-12-02 12:32:59
# Last Modified: 2018-12-02 12:34:22
*   Description: ---
*
************************************************/
...
public:
    ...
    {
        cout<<"基类获取名称参数："+name<<endl;
        return name;
    };
    virtual void setname(string name="teacher")
    {
        this->name = name;
    };
private:
    ...
    {
        return course;
    };                    //c++中，private函数也可以虚化重写，java中只能重写public和protect方法//继承时可以修改访问限制符
    string name;
protected:
    string course;
};
...
public:
    ...
    Headmaster()
    {
        cout<<"派生类无参构造函数：headmaster"<<endl;
    }; //派生类的构造函数  调用前会自动调用基类的默认构造函数
    ...
    virtual Student& getstudent() override
    {
        return student;
    };       //记得添加override,因为基类改变,派生类没改变会报错,这样提醒修改派生类
    virtual void setstudent(Student& students) override            //应该重写,重载方法的,所有版本。虚函数可以不重写，重写不重写都保持函数虚化特性
    {
        this->student = student;
    };
    virtual string getcourse() override
    {
        return course;
    };           //重写基类的private函数，并修改访问修饰符
